...
const FilteredListOfDoctors = () => {
  ...
  const filterDoctors = (doctor, department, allDoctorData) => {
    ...
    if (department && doctor)
      return allDoctorData.filter(
        (dr) => doctor === dr.name && department === dr.department
      );
    if (!department && !doctor) return allDoctorData;
  };
    ...
    <div>
      <div className="department-dropdown">
        <DropdownFilter
          ...
          onChangeHandler={handleDepartmentSelection}
        />
        <DropdownFilter
          label="Doctors"
          options={personOptions}
          onChangeHandler={handleDoctorSelection}
        />
      </div>
      {allDoctorData && (
    
          <DoctorInformation
            filteredDoctors={filterDoctors(doctor, department, allDoctorData)}
          />
